...
public class CreateReportUseCase implements CreateReportInputPort {
    ...
    public ConsolidatedResponse generateAntecedentReport(RequestSearch requestSearch) throws Exception {
        ...
        ApiResponse publicSpendingWatchdogResponse = antecedentReportClient.getInfoPublicSpendingWatchdog(token, requestSearch.getDocumentType(), requestSearch.getDocumentNumber());
        ApiResponse attorneyOfficeResponse = antecedentReportClient.getInfoDisciplinaryAntecedents(token, requestSearch.getDocumentType(), requestSearch.getDocumentNumber());
        ApiResponse policeAntecedentsResponse = antecedentReportClient.getInfoPoliceAntecedents(token, requestSearch.getDocumentType(), requestSearch.getDocumentNumber());
        ...
        ConsolidatedResponse consolidatedResponse = createUtilOutputPort.createConsolidatedResponse(policeAntecedentsResponse);
    ...
    public ConsolidatedResponse generateSearchPersonReport(RequestSearch requestSearch) throws Exception {
        ApiResponse personVotingResponse = searchPersonClient.getInfoPersonVoting(token, requestSearch.getDocumentType(), requestSearch.getDocumentNumber());
        ...
        RequestPdfEmail requestPdfEmail = createUtilOutputPort.createRequestPdfEmail(requestSearch.getName(),requestSearch.getEmail(), consolidatedBase64Report);
        //emailNotificationClient.sendPdfByEmail(requestPdfEmail);
        ConsolidatedResponse consolidatedResponse = createUtilOutputPort.createConsolidatedResponse(personVotingResponse);
